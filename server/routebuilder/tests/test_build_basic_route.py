...
def test_join_ways_different_directions(grid, l1, l2, d):
    ...
    assert route == rt.RouteSegment(
        length=20, start=0, linear='yes', appendices = [],
        main=[rt.WaySegment(
                length=10, start=0,
                ways=[rt.BaseWay(osm_id=1, tags={}, length=10, start=0,
                                 direction=0, role='', geom=g.line('12'))]
                ),
              rt.WaySegment(
                length=10, start=10,
                ways=[rt.BaseWay(osm_id=2, tags={}, length=10, start=10,
...
def test_single_relation(grid):
    ...
    members = [
        rt.RouteSegment(
            length=10, start=0, linear='sorted', appendices = [],
            main=[rt.WaySegment(
                length=10, start=0,
    ...
    assert route == rt.RouteSegment(
        length=10, start=0, linear='yes', appendices = [],
    ...
    assert route == rt.RouteSegment(
        ...
        main=[rt.WaySegment(
              ...
              rt.WaySegment(
                length=10, start=10,
                ways=[
                      rt.BaseWay(osm_id=2, tags={}, length=10, start=10,
